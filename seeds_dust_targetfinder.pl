...
while (<>) {
  ...
  if ($dust || $targetfinder) {
    ($s_fh, $s_fn) = tempfile("stmpXXXXXXX", DIR => "."); print $s_fh ">small\n$smallrna";   close $s_fh;
    ($t_fh, $t_fn) = tempfile("ttmpXXXXXXX", DIR => "."); print $t_fh ">target\n$targetrna"; close $t_fh;
  }
...
sub is_1mm_6mer {
  my ($srna, $trna) = @_;
  return aindex( &revcomp(substr($srna,1,6)), ["S1 D0 I0"], $trna )
}

sub is_1bulge_6mer {
  my ($srna, $trna) = @_;
  return aindex( &revcomp(substr($srna,1,6)), ["S0 D1 I1"], $trna )
}

sub is_2mm_7mer_m8 {
  my ($srna, $trna) = @_;
  return aindex( &revcomp(substr($srna,1,7)), ["S2 D0 I0"], $trna )
}
